<?php
  ...
  if (isset($_POST['submit'])) {
    ...
    foreach ($_POST['CompImageID'] as $index => $comp_image_id) {
        ...
        // check if the image already has points in the tbl_comp_results table for this member
        $sql = "SELECT * FROM `tbl_comp_results` WHERE `fldCompImageID` = ? AND `fldMemberID` = ?";
        ...
        if ($result->num_rows > 0) {
          $sql = "UPDATE `tbl_comp_results` SET `fldPoints` = ? WHERE `fldCompImageID` = ? AND `fldMemberID` = ?";
          $stmt = $mysqli->prepare($sql);
        ...
        // if the image doesn't have points for this member, insert the new points
          ...
          $sql = "INSERT INTO `tbl_comp_results` (`fldCompImageID`, `fldPoints`, `fldMemberID`) VALUES (?, ?, ?)";
          $stmt = $mysqli->prepare($sql);
    ...
    $sql = "UPDATE `tbl_comp_results`, temp_table
            SET `tbl_comp_results`.`fldTotalPoints` = temp_table.total_points
            WHERE `tbl_comp_results`.`fldCompImageID` = temp_table.fldCompImageID";
