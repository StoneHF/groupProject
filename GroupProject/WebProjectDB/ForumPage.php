...
<body>
    ...
    <div id="forum-list">
        ...
        <form action="process_forum.php" method="post"> <!-- create form with process_forum action -->
        <?php
            $sql = "SELECT * FROM `gnnwebsitedb`.`tbl_forums` ORDER BY `fldForumName` ASC"; //select all forums in ascending order from forum table
    ...
    $sql = "SELECT * FROM `gnnwebsitedb`.`tbl_temp_forum_link`"; //check temp table for any links
    $result = $mysqli->query($sql);
    ...
    <div id="forum-post-list"> <!-- area for forum posts being created -->
        <div id="posts-header">
            ...
            if ($tempForum == true){ //if there is a record in temp table
                $sql = "SELECT * FROM `gnnwebsitedb`.`tbl_temp_forum_link`"; //sql query to get record again from temp table
                $result = $mysqli->query($sql);
                while($row = $result->fetch_assoc()){
                    $forumLink = $row['fldLink']; //save temp record as a variable

                    $sql2 = "SELECT * FROM `gnnwebsitedb`.`tbl_forums` WHERE `fldForumID` = $forumLink"; //select all from forum table with the same forum id as the variable
                ...
                <form action="process_forum_2.php" method="post"> <!-- create form for letting users create posts -->
        ...
        <?php
            ...
            if ($tempForum == true){ //if temp forum has record in it
                ...
                $sql = "SELECT * FROM `gnnwebsitedb`.`tbl_temp_forum_link`"; //get record again from temp table
                $result = $mysqli->query($sql);
                while($row = $result->fetch_assoc()){
                    ...
                    $sql2 = "SELECT * FROM `gnnwebsitedb`.`tbl_forum_posts` WHERE `fldForumID` = $forumLink"; //search forum table for record with variable from temp table
                    $result2 = $mysqli->query($sql2);
                ...
                $sql = "DELETE FROM `gnnwebsitedb`.`tbl_temp_forum_link` WHERE `fldLink` = $forumLink"; //deletes record from temp table
                $result = $mysqli->query($sql);
            }
